...
commit) — or, for the {{FIG:aud_absence}} cells that assert the *absence* of a rule, against chronology, since
non-adoption leaves no record to cite — with **{{FIG:aud_fetched}} of the {{FIG:aud_specified}} chain cells
...
changed it. **{{FIG:n_class_ii}} axes: the initial block subsidy and the target block spacing.**
...
for the v0.1.0 reference only: the three axis perturbations carry no information over one or {{FIG:nov_spec}}
...
is by warrant, not by label. Of the {{FIG:aud_absence}}, seven carry the value `none` and the two Schnorr
...
full cell-level record, including the criterion, cited source and confidence for each of the {{FIG:n_cells}}
cells, {{FIG:n_unspecified}} of which are unspecified), `comparison.csv` (the summary), and `axis_matrix.csv` (the raw axis values); the figure
...
raw 32-byte SHA-256 of its contents, with no separator, in sorted file-name order, so those {{FIG:n_tables_manifest}} files are pinned without
