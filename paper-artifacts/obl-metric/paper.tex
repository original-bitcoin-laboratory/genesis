...
\textbf{The replication package is archived at
\url{https://doi.org/10.5281/zenodo.21964447}}, and the same artifacts
...
\texttt{obl-metric-v1.0.0} (commit
\texttt{a15e2b60a5ac60690f680bd027fe95bab12d90f0}). \emph{The DOI is the
durable address and the tag is the immutable one; the repository path is
a convenience that may move.} \textbf{Every numerical figure quoted in
this paper is interpolated from the engine's output when the manuscript
is built, not typed}, so the reported values are re-derived on every
build rather than compared against the engine as of a date. The digests
below identify the engine outputs from which the reported results can be
