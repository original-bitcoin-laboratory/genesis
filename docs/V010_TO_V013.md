...
The lab's earlier work compares the 15 November 2008 pre-release with the January 2009 v0.1.0 (announced 8 January). This
continues that line forward by one step, to the next artifact that survives: **v0.1.3**, dated
13 January 2009. **The archive we hold as `bitcoin-0.1.0` is v0.1.1 by its bytes; no v0.1.0 or v0.1.2 archive survives to us** — the jump we can execute is
0.1.1 (labelled 0.1.0) → 0.1.3, and the Satoshi Nakamoto Institute marks even the 0.1.3 date with a question mark.
...
> distinction matters: *"not released"* and *"released and lost"* are different facts, and only the
> second is true. **Nothing in this document's finding depends on it** — the diff is between the two
> codebases we hold. Established from the SourceForge file listing.
>
...
> source. **This dates availability, not release:** a capture shows what was on offer that day, not
...
> 0.1.3 → 0.3.19, established from a CDX sweep of the Internet Archive's captures of bitcoin.org.
