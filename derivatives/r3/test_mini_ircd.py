...
async def _register(host, port, nick):
    reader, writer = await asyncio.open_connection(host, port)
    await _read_until(reader, "Found your hostname")
    writer.write(f"NICK {nick}\r\n".encode())
    writer.write(f"USER {nick} 8 * :{nick}\r\n".encode())
    await writer.drain()
    await _read_until(reader, " 004 ")
    writer.write(b"JOIN #bitcoin\r\n")
    await writer.drain()
...
async def _scenario():
    ...
    async with server:
        ra, wa = await _register("127.0.0.1", port, "uAAAA1111")
        rb, wb = await _register("127.0.0.1", port, "uBBBB2222")
...
def test_two_nodes_discover_each_other():
    a_who, b_who = asyncio.run(_scenario())
