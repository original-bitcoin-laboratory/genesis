...
# value overflow: two outputs whose int64 sum wraps (the August 2010 shape, derivatives/overflow/);
# exact arithmetic sees outputs far above the input and rejects it as inflation
BIG = 9_223_372_036_854_277_039
ov = Tx(1, [TxIn(SPENDABLE_TXID, 0, b"", 0xFFFFFFFF)], [TxOut(BIG, bytes([0x51])), TxOut(BIG, bytes([0x52]))], 0)
ov.vin[0].script = cscript.assemble([sign(priv, [pub, "OP_CHECKSIG"], ov, 0)])
invalid.append(("value_overflow",) + make_invalid("value_overflow", [coinbase(3, subsidy_at(3), P2PK), ov]))
