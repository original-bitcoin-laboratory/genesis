...
|---|---|---|
| `2026-07-31/2026-07-31_223114` | 31 Jul 22:31 | **Setup / connectivity** — each VM `ping`s the other across `172.20.0.0/24` (A↔B, 0% loss) before any node starts |
| `2026-08-01/2026-08-01_034539` | 1 Aug 03:45 | **R4a byte-identical result** — both nodes' `Get-FileHash blk0001.dat` = `AAF08AA3…6DBEBC4C` (the `aaf08aa3…` in the R4a manifest): the two separate node instances agree on the chain byte for byte |
| `2026-08-02/2026-08-02_044409` | 2 Aug 04:44 | **R4b reorg, as the wallet shows it** — node A's top transaction is `0/unconfirmed — Generated (not accepted)` (its orphaned, self-mined block), both nodes at 16 blocks (height 15) |
| `2026-08-02/2026-08-02_044538` | 2 Aug 04:45 | **R4b `REORGANIZE` confirmed** — `Select-String REORGANIZE C:\obl\debug.log` on node A returns `debug.log:542:*** REORGANIZE ***`, alongside the orphaned-block wallet entry |
| `2026-08-02/2026-08-02_044616` | 2 Aug 04:46 | **R4b on-disk divergence** — post-reorg `Get-FileHash` shows the two nodes' `blk0001.dat` now **differ** (`909852B7…D8DAF0` on A vs `48A84CE7…60BDCD9` on B, the exact values in the R4b manifest): A carries the retained orphan (+223 B = one block) |
| `2026-08-02/2026-08-02_173207` | 2 Aug 17:32 | **Current** — both nodes at **26 blocks** (height 25), node B still `Generating`; node A still retains its reorg orphan at the top of the wallet. The run continues toward R4c's ~101-block coinbase maturity |
